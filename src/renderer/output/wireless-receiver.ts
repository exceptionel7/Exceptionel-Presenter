...
export interface ReceiverEvents {
  /** A real remote track arrived. This is the only honest signal that video exists. */
...
export function createWirelessReceiver(events: ReceiverEvents): WirelessReceiver {
  ...
  const create = (sessionId: string): Peer => {
    ...
    const peer: Peer = { connection, stream: null, statsTimer: null, previous: null };
    ...
    connection.ontrack = (event) => {
      /*
       * The moment that actually matters. `event.streams[0]` is the phone's live MediaStream;
       * until this fires there is no video, which is why the state machine reaches `connected`
       * only from here and never from a completed handshake.
       */
    ...
    connection.onconnectionstatechange = () => {
      ...
      if (state === 'connected') startStats(sessionId, peer);
